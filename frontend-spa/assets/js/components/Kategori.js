const Kategori = {
    ...
    <div>
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-gray-800 font-bold text-xl">Kategori</h1>
                <p class="text-gray-400 text-sm mt-0.5">Kelola kategori barang inventaris</p>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-100">
                            ...
                            <th class="text-left text-gray-500 font-semibold px-6 py-3.5 text-xs uppercase tracking-wider">Nama Kategori</th>
                            <th class="text-left text-gray-500 font-semibold px-6 py-3.5 text-xs uppercase tracking-wider">Deskripsi</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        <tr v-if="loading">
                            <td :colspan="userRole === 'admin' ? 4 : 3" class="text-center py-12">
                        ...
                        <tr v-else v-for="(item, index) in filteredKategori" :key="item.id_kategori"
                            class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 text-gray-400 text-xs">{{ index + 1 }}</td>
                            <td class="px-6 py-4">
                                <span class="bg-purple-50 text-purple-600 text-xs font-medium px-3 py-1.5 rounded-lg">
                                    {{ item.nama_kategori }}
                                </span>
                            </td>
        ...
        <div v-if="showModal"
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
                ...
                <div class="px-6 py-5 space-y-4">
                    <div>
                        <label class="block text-gray-600 text-sm font-medium mb-1.5">Nama Kategori <span class="text-red-500">*</span></label>
                        <input type="text" v-model="form.nama_kategori" placeholder="Masukkan nama kategori"
                    ...
                    <div>
                        ...
                        <textarea v-model="form.deskripsi" placeholder="Deskripsi kategori (opsional)" rows="3"
    ...
    computed: {
        filteredKategori() {
            ...
            return this.kategoriList.filter(k =>
                k.nama_kategori.toLowerCase().includes(this.search.toLowerCase())
            );
        }
    },
    mounted() {
    ...
    methods: {
        ...
        simpan() {
            if (!this.form.nama_kategori) {
                this.errorMsg = 'Nama kategori wajib diisi!';
                return;
            }
            this.loadingSave = true;
            const request = this.isEdit
                ? axios.put(apiUrl + '/api/kategori/' + this.form.id_kategori, this.form)
                : axios.post(apiUrl + '/api/kategori', this.form);
        ...
        hapus(id) {
            if (confirm('Yakin ingin menghapus kategori ini?')) {
                axios.delete(apiUrl + '/api/kategori/' + id)
                    .then(() => { this.loadData(); })
                    .catch(() => { alert('Gagal menghapus. Mungkin kategori masih digunakan oleh barang.'); });
            }
        }
    }
};